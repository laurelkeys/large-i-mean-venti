...
#include "OctreePoint.hh"

class Octree {
    private:
        // Physical position/size (implicitly defines the bounding box of this node).
        Vec3 origin; // The physical center of this node
        Vec3 halfDimension; // Half the width/height/depth of this node

        // The tree has up to eight children and can additionally store a point,
        // though in many applications only the leaves will store data.
    ...
    public:
        ...
        int getOctantContainingPoint(const Vec3 &point) const {
            int oct = 0;
            if (point.x >= origin.x) oct |= 4; // 0b100
            if (point.y >= origin.y) oct |= 2; // 0b010
            if (point.z >= origin.z) oct |= 1; // 0b001
            return oct;
        }
        ...
        bool isLeafNode() const {
            ...
            // We are a leaf iff we have no children. Since we either have none, 
            // or all eight, it is sufficient to just check the first.
        ...
        void insert(OctreePoint *point) {
            if (isLeafNode()) {
                ...
                // We will split this node so that it has 8 child octants and then 
                // insert the old data that was here, along with this new data point.

                // Save the data point that was here for a later re-insert
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    newOrigin.x += halfDimension.x * (i & 4 ? .5f : -.5f); // i & 0b100
                    newOrigin.y += halfDimension.y * (i & 2 ? .5f : -.5f); // i & 0b010
                    newOrigin.z += halfDimension.z * (i & 1 ? .5f : -.5f); // i & 0b001
                ...
                // Insert recursively into the appropriate child octant.
