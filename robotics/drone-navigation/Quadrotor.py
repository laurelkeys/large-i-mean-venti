...
class Quadrotor():
    def __init__(self, x=0, y=0, z=0, roll=0, pitch=0, yaw=0, size=0.25, show_animation=True):
        self.p1 = np.array([ size / 2,      0,    0, 1])
        self.p2 = np.array([-size / 2,      0,    0, 1])
        self.p3 = np.array([     0,     size / 2, 0, 1])
        self.p4 = np.array([     0,    -size / 2, 0, 1])
    ...
    def update_pose(self, x, y, z, roll, pitch, yaw):
        ...
        self.x_data.append(x); self.y_data.append(y); self.z_data.append(z)
        if self.show_animation:
            self.plot()
