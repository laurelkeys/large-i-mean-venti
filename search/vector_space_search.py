# ref.: https://boyter.org/2010/08/build-vector-space-search-engine-python/

class VectorCompare:
    def magnitude(self, concordance):
        assert isinstance(concordance, dict), "Supplied argument should be of type 'dict'"
        return sum(count ** 2 for count in concordance.values()) ** 0.5

    def relation(self, concordance1, concordance2):
        assert isinstance(concordance1, dict), "Supplied argument 1 should be of type 'dict'"
        assert isinstance(concordance2, dict), "Supplied argument 2 should be of type 'dict'"
        top_value = sum(count1 * concordance2[word1] 
                        for word1, count1 in concordance1.items() 
                        if word1 in concordance2)
        magnitude_product = self.magnitude(concordance1) * self.magnitude(concordance2)
        return top_value / magnitude_product if magnitude_product != 0 else 0

    def concordance(self, document):
        assert isinstance(document, str), "Supplied argument should be of type 'str'"
        con = {}
